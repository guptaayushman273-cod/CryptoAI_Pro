...
def make_decision(market_score, news_score, fee_ok, indicators):
    """
    Decide whether the current market setup is suitable for buying.
    ...
    if any(pd.isna(value) for value in values_to_check):
        return {
            ...
            "score": 0,
            "technical_confirmations": 0,
            "reason": "Indicator values contain NaN."
        }
    ...
    market_component = min(max(market_score, 0), 40)
    ...
    news_component = news_score * 3
    news_component = min(max(news_component, -15), 15)
    ...
    if 0.20 <= band_position <= 0.75:
        score += 5
        reasons.append("Price in usable Bollinger zone")
    ...
    if atr_percent < 0.10:
        ...
        volatility_ok = False
        reasons.append(
            f"Volatility too low ({atr_percent:.2f}%)"
        )

    elif atr_percent > MAX_BUY_ATR_PERCENT:
        score -= 15
        volatility_ok = False
        reasons.append(
            f"Volatility too high ({atr_percent:.2f}%)"
        )
        ...
        confirmations += 1
        reasons.append(
            f"Usable volatility ({atr_percent:.2f}%)"
        )
    ...
    strong_trend = bullish_ema and price_above_ema20

    strong_momentum = bullish_macd and rsi_good
